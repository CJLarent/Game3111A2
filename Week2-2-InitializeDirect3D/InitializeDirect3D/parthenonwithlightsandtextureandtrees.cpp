...
void ShapesApp::Draw(const GameTimer& gt)
{
	...
	mCommandList->ClearRenderTargetView(CurrentBackBufferView(), Colors::DarkMagenta, 0, nullptr);
...
void ShapesApp::UpdateMainPassCB(const GameTimer& gt)
{
	...
	mMainPassCB.AmbientLight = { 0.25f, 0.32, 0.35f, 0.45f };
	mMainPassCB.Lights[0].Direction = { 0.57735f, -0.57735f, 0.57735f };
	mMainPassCB.Lights[0].Strength = { 0.4f, 0.4f, 0.4f };
	mMainPassCB.Lights[1].Direction = { -0.57735f, -0.57735f, 0.57735f };
	mMainPassCB.Lights[1].Strength = { 0.2f, 0.2f, 0.2f };
	mMainPassCB.Lights[2].Direction = { 0.0f, -0.900f, -0.900f };
	mMainPassCB.Lights[2].Strength = { 0.10f, 0.10f, 0.10f };
	...
	mMainPassCB.Lights[4].SpotPower = .0f;
...
//step6
void ShapesApp::LoadTextures() // added custom textures Wall2.dds and Waltex.dds 
{
	...
	stoneTex->Filename = L"../../Textures/Wall2.dds";
	ThrowIfFailed(DirectX::CreateDDSTextureFromFile12(md3dDevice.Get(),
	...
	tileTex->Filename = L"../../Textures/Wall2.dds";
	ThrowIfFailed(DirectX::CreateDDSTextureFromFile12(md3dDevice.Get(),
	...
	skullTex->Filename = L"../../Textures/Waltex.dds";
	ThrowIfFailed(DirectX::CreateDDSTextureFromFile12(md3dDevice.Get(),
	...
	//  ground texture added from the common folder 
...
void ShapesApp::BuildShapeGeometry()
{
	...
	GeometryGenerator::MeshData ground = geoGen.CreateBox(10.0f, 0.2f, 10.0f, 3);
	/*
	Step1
	...
	UINT groundVertexOffset = (UINT)cylinder.Vertices.size() + cylinderVertexOffset;
	/*
	Step2
	...
	UINT groundIndexOffset = CylinderIndexOffset + (UINT)cylinder.Indices32.size();
	...
	auto totalVertexCount = box.Vertices.size() + box2.Vertices.size() + cylinder.Vertices.size() + ground.Vertices.size();
...
void ShapesApp::BuildRenderItems()
{
	...
	XMStoreFloat4x4(&groundRitem->World, XMMatrixScaling(10.0f, 1.0f, 10.0f) * XMMatrixTranslation(0.0f, -1.0f, 0.0f));
	XMStoreFloat4x4(&groundRitem->TexTransform, XMMatrixScaling(10.0f, 20.0f, 10.0f));

	groundRitem->ObjCBIndex = 3;
	groundRitem->Geo = mGeometries["shapeGeo"].get();
	groundRitem->Mat = mMaterials["grassMat"].get();
	...
	for (int i = 0; i < 4; i++)// changed the loop to draw less cylinders 
	{
		for (int j = 0; j < 9; j++)
		{
			rItem[i][j] = std::make_unique<RenderItem>();
			XMStoreFloat4x4(&rItem[i][j]->World, XMMatrixScaling(0.5f, 1.0, 0.5f) * XMMatrixTranslation(-3.5 + (i*2), 4.5f, -3.0f + ((j*2))));
			//step24
